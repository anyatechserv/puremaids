...
interface Payment {
  id: string; booking_id: string; amount_pence: number;
  refund_amount_pence: number; status: string; created_at: string;
...
export default function AdminRefundsPage() {
  ...
  async function handleRefund() {
    ...
    try {
      ...
      if (pence <= 0 || pence > rem) { setErr(`Amount must be between £0.01 and ${fmt(rem)}`); setProcessing(false); return; }
      await requestRefund(session.access_token, {
        paymentId: modal.id, amountPence: pence, reason,
        mode: pence >= rem ? 'full' : 'partial',
      });
      setSuccess(`Refund of ${fmt(pence)} processed.`);
      setModal(null);
      await load();
    } catch (e) {
  ...
  if (!isAdmin) return (
    <div className="flex min-h-screen items-center justify-center p-4">
      <div className="card p-8 text-center max-w-sm">
        ...
        <p className="mt-2 text-gray-600">Admin access required to view this page.</p>
      </div>
    </div>
  );
    ...
    <>
      ...
      <main id="main-content" className="min-h-screen bg-gray-50 pt-20">
        <div className="container py-10">
          ...
          {success && <div className="alert alert-success mt-4 max-w-md" role="status">{success}</div>}
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    {['Booking','Date','Charged','Refunded','Refundable','Status','Action'].map(h => (
                ...
                <tbody className="divide-y divide-gray-200">
                  {payments.length === 0
                    ? <tr><td colSpan={7} className="px-5 py-10 text-center text-sm text-gray-400">No refundable payments found.</td></tr>
                    : payments.map(p => {
                        ...
                        return (
                          <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-5 py-4 text-sm text-gray-600 whitespace-nowrap">{new Date(p.created_at).toLocaleDateString('en-GB')}</td>
                            <td className="px-5 py-4 text-sm font-semibold">{fmt(p.amount_pence)}</td>
                            <td className="px-5 py-4 text-sm text-amber-600">{refunded > 0 ? fmt(refunded) : '—'}</td>
                            <td className="px-5 py-4 text-sm font-semibold">{fmt(rem)}</td>
                            <td className="px-5 py-4"><span className="badge-green">{p.status}</span></td>
                            <td className="px-5 py-4 text-sm">
                              {rem > 0
                                ? <button onClick={() => openModal(p)} className="font-semibold text-red-600 hover:text-red-700 transition-colors">Refund</button>
      ...
      {/* Refund modal */}
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-fade-in" role="dialog" aria-modal="true" aria-labelledby="refund-title">
          <div className="card w-full max-w-md p-6 animate-slide-up">
            <h2 id="refund-title" className="font-display text-lg font-bold text-gray-900">Process Refund</h2>
            <p className="mt-1 text-sm text-gray-500">
              Booking {modal.bookings?.[0]?.reference ?? '—'} · Charged: {fmt(modal.amount_pence)}
            </p>
            {err && <div className="alert alert-error mt-4" role="alert">{err}</div>}
            <div className="mt-4 space-y-4">
              <div>
                <label htmlFor="refund-amount" className="label">Refund amount (£)</label>
                <input id="refund-amount" type="number" step="0.01" min="0.01" max={(modal.amount_pence / 100).toFixed(2)} value={amount} onChange={e => setAmount(e.target.value)} className="input" />
              </div>
              <div>
